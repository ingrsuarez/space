...
@section('content')
  ...
  <div class="card mx-4">
    ...
    <div class="card-body">
      ...
      <!-- Modal -->
      <form id="lock" action="{{ route('appointment.storeServiceLock') }}" method="POST">
        @method('POST')
        @csrf
            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                <div class="input-group mb-3">
                  ...
                  <input type="text" class="form-control" aria-label="Default" aria-describedby="inputGroup-sizing-default" id="userEvent" value="{{ucfirst($service->name)}}" readonly>
      ...
      {{-- EMPTY DATE CLICKED --}}
      <form id="actualizar-ficha" action="{{ route('appointmentService.store') }}" method="POST">
        @method('POST')
        @csrf
            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                <div class="input-group mb-3">
                  ...
                  <input type="text" class="form-control" aria-label="Default" aria-describedby="inputGroup-sizing-default" id="user" value="{{ucfirst($service->name)}}" readonly>
                  <input type="hidden" name="service_id" value="{{$service->id}}">
                </div>
                <div class="input-group mb-3">
                  <div class="input-group-prepend">
                ...
                @livewire('find-patients',['service' => $service])
              </div>
              <div class="modal-footer">
        ...
        <div class="modal fade" id="eventModal" tabindex="-1" role="dialog" aria-labelledby="calendarModalLabel" aria-hidden="true">
          <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
              ...
              <form id="eventAction" action="{{ route('appointmentService.cancel') }}" method="POST">
                @method('POST')
                @csrf
                <div class="modal-body">
                  <div class="input-group mb-3">
              ...
              @if (!$user->hasRole('profesional'))
                <form id="sendToWatingList" action="{{ route('appointment.toWaitingService') }}" method="POST">
                @method('POST')
                @csrf
                  <div class="modal-body">
                    <input type="hidden" id="institution" name="institution_id" value="{{$institution->id}}">
      ...
      <form id="sendConfirmation" action="{{ route('waService.send') }}" method="POST">
        @method('POST')
        @csrf
      ...
      {{-- EVENT CLICKED --}}
      <form id="eventReschedule" action="{{ route('appointmentService.reschedule') }}" method="POST">
        @method('POST')
        @csrf
        <input type="hidden" id="institutionEvent" name="institution_id" value="{{$institution->id}}">
        <input type="hidden" id="serviceEvent" name="service_id" value="{{$service->id}}">
